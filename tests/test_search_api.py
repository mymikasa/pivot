...
def test_search_returns_ranked_results():
    mock_results = [
        _mock_node_with_score("Pivot 是一个知识库管理平台", 0.95, {
            "kb_id": 1, "pivot_document_id": 10, "chunk_index": 0,
            "section_title": "简介", "section_path": "/简介/",
            "filename": "intro.md", "content_type": "text/markdown",
            "token_count": 5,
        }),
        _mock_node_with_score("向量检索使用 Milvus 数据库", 0.72, {
            "kb_id": 1, "pivot_document_id": 10, "chunk_index": 1,
            "section_title": "架构", "section_path": "/架构/",
            "filename": "intro.md", "content_type": "text/markdown",
            "token_count": 5,
        }),
    ]

    with patch("src.pipeline.search._build_embedding") as mock_embed, \
         patch("src.pipeline.search._build_vector_store") as mock_vs, \
         patch("src.pipeline.search.VectorStoreIndex") as mock_index_cls:
...
def _setup_search_mocks(stack, *, with_reranker=False):
    ...
    mock_embed = stack.enter_context(patch("src.pipeline.search._build_embedding"))
    mock_vs = stack.enter_context(patch("src.pipeline.search._build_vector_store"))
    mock_index_cls = stack.enter_context(patch("src.pipeline.search.VectorStoreIndex"))
    mock_embed.return_value = MagicMock()
    mock_vs.return_value = MagicMock()
    mock_index = MagicMock()
    mock_index_cls.from_vector_store.return_value = mock_index
    ...
    if with_reranker:
        mock_build_reranker = stack.enter_context(patch("src.pipeline.search.build_reranker"))
...
def test_search_default_no_rerank_uses_top_k():
    ...
    mock_results = [
        _mock_node_with_score("chunk A", 0.9, {
            "kb_id": 1, "pivot_document_id": 10, "chunk_index": 0,
            "filename": "a.md", "content_type": "text/markdown", "token_count": 5,
        }),
    ]
...
def test_search_rerank_without_top_k_uses_default_candidate():
    ...
    reranked_results = [
        SearchResult(
            chunk_index=0, document_id=10, kb_id=1, score=0.99,
            content="chunk A", token_count=5, source_page=None,
            section_title=None, section_path=None,
            filename="a.md", content_type="text/markdown",
        ),
    ]

    with ExitStack() as stack:
        _, _, _, mock_index, mock_retriever, mock_build_reranker = _setup_search_mocks(stack, with_reranker=True)
...
def test_search_rerank_with_explicit_top_k():
    ...
    candidates = [
        SearchResult(
            chunk_index=i, document_id=10, kb_id=1,
            score=0.9 - i * 0.1, content=f"chunk {i}",
            token_count=5, source_page=None,
            section_title=None, section_path=None,
            filename="a.md", content_type="text/markdown",
        )
        for i in range(8)
    ...
    with ExitStack() as stack:
        _, _, _, mock_index, mock_retriever, mock_build_reranker = _setup_search_mocks(stack, with_reranker=True)
        mock_nodes = [
            _mock_node_with_score(c.content, c.score, {
                "kb_id": c.kb_id, "pivot_document_id": c.document_id,
                "chunk_index": c.chunk_index, "filename": c.filename,
                "content_type": c.content_type, "token_count": c.token_count,
            })
            for c in candidates
        ]
        mock_retriever.retrieve.return_value = mock_nodes
        ...
        response = client.post(
            "/api/v1/search",
            json={"query": "测试", "kb_id": 1, "top_k": 3, "rerank": True, "rerank_top_k": 8},
            headers=auth_headers(),
        )
...
def test_search_rerank_changes_result_order():
    ...
    original_order = [
        SearchResult(
            chunk_index=0, document_id=10, kb_id=1, score=0.9,
            content="高分 chunk", token_count=5, source_page=None,
            section_title=None, section_path=None,
            filename="a.md", content_type="text/markdown",
        ),
        SearchResult(
            chunk_index=1, document_id=10, kb_id=1, score=0.7,
            content="低分 chunk", token_count=5, source_page=None,
            section_title=None, section_path=None,
            filename="a.md", content_type="text/markdown",
        ),
    ]
...
def test_search_rerank_unconfigured_returns_503():
    ...
    with ExitStack() as stack:
        ...
        mock_retriever.retrieve.return_value = [
            _mock_node_with_score("chunk", 0.5, {
                "kb_id": 1, "pivot_document_id": 10, "chunk_index": 0,
                "filename": "a.md", "content_type": "text/markdown", "token_count": 5,
            }),
        ]
...
def test_search_rerank_top_k_less_than_top_k_returns_422():
    ...
    response = client.post(
        "/api/v1/search",
        json={"query": "测试", "kb_id": 1, "top_k": 10, "rerank": True, "rerank_top_k": 5},
        headers=auth_headers(),
    )
