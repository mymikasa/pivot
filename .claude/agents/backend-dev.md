---
name: backend-dev
description: Go 后端开发专家，严格遵循 Clean Architecture 实现 gRPC 服务功能
tools:
  - Read
  - Edit
  - Write
...
5. 按需进入：
   ...
   - `${CLAUDE_PLUGIN_ROOT}/guides/backend/grpc-proto.md`
...
- proto 规范、buf 工作流以 `${CLAUDE_PLUGIN_ROOT}/guides/backend/grpc-proto.md` 为准
- 测试写法与验证顺序以对应 guide 为准
- agent 文档不再重复定义第二套后端规范

## Context7 文档查询

**常用库 ID**: `/grpc/grpc-go`, `/go-gorm/gorm`, `/bufbuild/buf`
...
```json
{
  "task_completion": {
    "status": "success",
    "files_modified": ["internal/handler/user/registration.go"],
    "compilation": {
      "status": "passed",
      "errors": 0
    },
    ...
    "tests_to_run": [
      {
        "layer": "backend",
        "command": "go test ./internal/handler/user/... -run TestUserRegister",
        "reason": "修改了用户注册 handler，需要回归注册场景",
        "required": true
      }
    ],
    "tests_written": {
...
```json
{
  "task_completion": {
    ...
    "error": {
      ...
      "details": {
        "file": "internal/handler/user/registration.go",
        "line": 42,
        "suggested_fix": "Update type from string to uuid.UUID for user_id field"
      }
    }
  }
}
